import { people } from './ts6-data';
import { Person } from './ts6-solution';
...
test('Summary', () => {
	expect(bob.getSummary()).toBe('Bob is a vegetarian, is 110cm tall and has 78 years to live');
	expect(thomas.getSummary()).toBe('Thomas likes meat, is 121cm tall and has 77 years to live');
	expect(doris.getSummary()).toBe('Doris is a vegetarian, is 80cm tall and should be dead');
});
